...
export default function EncaissementImportResult({ batch, rows }: EncaissementImportResultProps) {
    const failedRows = rows.filter((r) => r.status === 'ECHEC_COMMIT');
    const insertedRows = rows.filter((r) => r.status === 'INSERE');
    const importedTotal = insertedRows.reduce((sum, r) => sum + (Number(r.raw.montant) || 0), 0);
            ...
            <Card title={`${batch.original_filename} — ${batch.etablissement?.nom_centre ?? ''} / ${batch.annee_scolaire?.nom ?? ''}`}>
                ...
                <div className="row mb-4">
                    <div className="col-md-4">
                        <div className="text-muted">Total Excel (toutes lignes)</div>
                        <div className="fs-5 fw-semibold">{excelTotal.toFixed(2)} DH</div>
                    </div>
                    <div className="col-md-4">
                        <div className="text-muted">Total importé</div>
                        <div className="fs-5 fw-semibold">{importedTotal.toFixed(2)} DH</div>
                    </div>
                    <div className="col-md-4">
                        <div className="text-muted">Différence</div>
                        <div className="fs-5 fw-semibold">{(excelTotal - importedTotal).toFixed(2)} DH</div>
                    </div>
                </div>
