...
interface PresenceImportPreviewProps {
    batch: ImportBatch;
...
/**
 * What will happen to the séance behind this line. Only rows that can
 * actually be committed get "Existante"/"À créer" — showing "À créer" on a
 * DOUBLON or ERREUR row would promise a séance that is never written.
 */
function seanceBadge(row: ImportRow) {
    ...
    return row.resolution?.existing_seance_id ? (
        <span className="badge bg-secondary-transparent">Existante</span>
    ) : (
        <span className="badge bg-info-transparent">À créer</span>
    );
}

export default function PresenceImportPreview({
    batch,
    rows,
    statusCounts,
    selectableRowIds,
    ...
    const handleDone = useCallback(() => {
        router.visit(`/backoffice/import/presences/${batch.id}/result`);
    }, [batch.id]);

    const progress = useCommitProgress(`/backoffice/import/presences/${batch.id}/commit`, handleDone);

    const totalRows = Object.values(statusCounts).reduce((sum, n) => sum + (n ?? 0), 0);

    function applyFilter(status: string) {
        router.get(
            `/backoffice/import/presences/${batch.id}/preview`,
            status === '' ? {} : { status },
    ...
    function retryFailedRows() {
        router.post(
            `/backoffice/import/presences/${batch.id}/retry-failed`,
            {},
            { preserveScroll: true },
        );
    }
